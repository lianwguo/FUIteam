...
csv <- paste0("data/MRIP_catch_dispo/catch_dispo_",years[1],wave[1],".csv",sep="")
...
for(w in 2:6) {
	...
	csv <- paste0("data/MRIP_catch_dispo/catch_dispo_",years[1],wave[w],".csv",sep="")
	
	new <- read.csv(csv,header=TRU4)
...
# second nested loop runs through each subsequent year and wave
 for(y in 2:length(years)) {
 	for(w in 1:length(wave)) {
		...
		csv <- paste0("data/MRIP_catch_dispo/catch_dispo_",years[y],wave[w],".csv",sep="")
...
write.csv(fullBind,"data/aggregatedMRIP/mripCatch_2004_2017.csv",row.names=FALSE)

#____________________________________________________
# bringing together the MRIP trip survey data

# Florida all in one file
FL_trip_2004_2015 <- read.csv("data/MRIP_trip_ZIP/original/mrip_survey_trip_2004_2015.csv") # all of florida
# Louisiana was broken up
LA_trip_2004 <- read.csv("data/MRIP_trip_ZIP/original/LA_trip_2004_2004_zipcode.csv")
LA_trip_2005_2010 <- read.csv("data/MRIP_trip_ZIP/original/LA_mrip_survey_trip_zipcode_2005_2010.csv") #this one seems wrong
LA_trip_2010_2015 <- read.csv("data/MRIP_trip_ZIP/original/mrip_survey_trip_2010_2015.csv") # part of Louisiana
